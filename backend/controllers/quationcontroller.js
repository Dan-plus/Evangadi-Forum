...
async function createQuestion(req, res) {
  try {
    const { title, description, tag } = req.body; // get question details
    const userid = req.user.userid; // user ID from authMiddleware
    ...
    let newId = lastRow.length === 0
      ? "q001"
      : "q" + (parseInt(lastRow[0].questionid.slice(1)) + 1).toString().padStart(3, "0");
    ...
    res.status(201).json({ message: "Question created successfully", questionId: newId });
  } catch (error) {
...
async function editQuestion(req, res) {
  try {
    ...
    await dbconnect.execute(
      ...
      [title || question.title, description || question.description, tag || question.tag, id]
    );
